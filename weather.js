...
//CURRENT weather
  ...
  $("#current-temperature").html(Math.round(response.currently.temperature) + "&#176;");
...
//DAILY FORECAST
  ...
  for(let i = 1; i<=6; i++) {
      ...
      markup += "<h3>" + icon(response.daily.data[i]) + "</h3>"; // Icon
      markup += "<h4>" + Math.round(response.daily.data[i].temperatureHigh) + "&#176;" + " | " + Math.round(response.daily.data[i].temperatureLow) + "&#176;" + "</h4>";  //High Temperature | Low Temperature
      markup += "<h5>" + response.daily.data[i].summary + "</h5>"; //Description of weather
      markup += "</div>"
  }
